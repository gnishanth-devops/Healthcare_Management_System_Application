pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    def nodejs_patient = docker.build("-t ${ECR_REPO_NAME}${IMAGE_TAG}","-f ${env.WORKSPACE}/src/appointment/Dockerfile .") 
                    def node_appointment = docker.build("-t ${ECR_REPO_NAME}:${IMAGE_TAG}","-f ${env.WORKSPACE}/src/patient/Dockerfile .") 
                }
