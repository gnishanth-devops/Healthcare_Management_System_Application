    ...
    stages {
        ...
        stage('Login to AWS ECR') {
            steps {
                script {
                    sh """
                       aws ecr get-login-password --region us-west-1 --profile jenkins | docker login --username AWS --password-stdin 539935451710.dkr.ecr.us-west-1.amazonaws.com/dev/ecs
                    """
                }
            }
        }
